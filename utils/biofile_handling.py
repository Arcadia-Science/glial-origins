...
class CellRangerFileGroup:
    ...
    def to_gxc(self, reference_genome, reference_annot, filename = '', overwrite = False):
        ...
        if filename == '':
            filename = self.matrix.filename + '.asgxc.csv'
        ...
        output = GxcFile(
            ...
            reference_annot = reference_annot, 
            sampledict = self.sampledict, 
            filename = filename
        )
...
class LoomFile(BioFile):
    ...
    def to_gxc(self, filename = '', overwrite = False):
        ...
        if filename == '':
            filename = self.filename + '.asgxc.tsv'
        ...
        if output_file.exists and not overwrite:
            print('GxcFile already exists at', output_file.path)
            return output_file
        else:
            with loompy.connect(self.path) as ds:
                dummy_df = pd.DataFrame(ds[:,:], columns = ds.ca.CellID)
                dummy_df.insert(0, 'gene_name', ds.ra.Gene)

            print('Creating gxc file at', output_file.path)    
            dummy_df.to_csv(output_file.path, sep = '\t', index = None)
            
            return output_file
    
    def get_idmm(self, id_type: str, overwrite = False):
        """Generates an idmm file from the LOOM file.
        
        Args:
            id_type (str): the name of the ID type extracted from the file, e.g. `'ensembl_id'`.
            overwrite (bool): whether or not to overwrite the file if it already exists.
        ...
        filename = self.filename + '.idmm.tsv'
        
        output_file = IdmmFile(
            sampledict = self.sampledict,
            sources = [self],
            kind = 'loom_idmm',
            filename = filename
        ...
        if output_file.exists and not overwrite:
            print('IdmmFile already exists at', output_file.path)
            return output_file
        else:
            with loompy.connect(self.path) as ds:
                annot_df = pd.DataFrame({'gene_name': ds.ra.Gene, id_type: ds.ra.Accession})
                
            print('Creating idmm file at', output_file.path)    
            annot_df.to_csv(output_file.path, sep = '\t', index = None)
            
            return output_file
    
    def get_cellannot(self, overwrite = False):
        ...
        filename = self.filename + '.cellannot.tsv'
        
        output_file = CellAnnotFile(
            sampledict = self.sampledict,
            sources = [self],
            filename = filename
        )
        ...
        if output_file.exists and not overwrite:
            print('CellAnnotFile already exists at', output_file.path)
            return output_file
        else:
            with loompy.connect(self.path) as ds:
                cellannot_df = pd.DataFrame({'cell_id': ds.ca.CellID, 'celltype': ds.ca.Clusters})
                
            print('Creating cellannot file at', output_file.path)    
            cellannot_df.to_csv(output_file.path, sep = '\t', index = None)
